...
// function to recalculate transformation matrix on key press
void display() {
	int i,j;
	...
	if(flag!=0) //&& checkin(look.x,look.y,look.z))
	{
		...
		if(flag==4)	{	//Slide the camera to the left
		...
		if(flag==5) {	// slide the camera to the right
		...
		if(flag==6) {	// slide the camera down
		...
		if(flag==8&&angle>10) {                         //	change angle for left rotation
			angle=angle-1;
		}
		
		if(flag==9&&angle<90) {
			angle=angle+1;     							// change angle for right rotation
		}
		
		if (flag ==11) {		// rotate left
			vec = sqrt((look.x-eye.x)*(look.x-eye.x) + (look.z-eye.z)*(look.z-eye.z));
			theta+=0.001;
	...
	// creation of components of the room
	glLoadIdentity();
